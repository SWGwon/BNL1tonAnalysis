...
AppConfig::AppConfig()
    : inputFileName(""), outputFilePath(""),
      ...
      eventNumber(10000), triggerType(0) // default triggerType: top paddle
{}
...
AppConfig AppConfig::parseArgs(int argc, char **argv) {
    ...
    if (config.inputFileName.empty() || config.outputFilePath.empty()) {
        ...
        std::cout << "  -i <inputFilePath>          Path to the input file."
                  << std::endl;
        std::cout << "  -o <outputFilePath>         Path to the output file."
                  << std::endl;
        std::cout << "  -n <eventNumber>            Event number to process."
                  << std::endl;
        std::cout << "  -t <triggerType>            Trigger type:" << std::endl;
        std::cout << "                              0: tp" << std::endl;
        std::cout << "                              1: alpha" << std::endl;
